...
## How to run it
1. Set up the tutor: follow **`chatgpt-setup.md`** (paste `tutor-prompt.md` as instructions; upload `coverage-map.md`, `mastery-tracker.md`, all of `study-packs/` + `reference/`).
2. Type **"let's start"** → short diagnostic → then **"let's continue"** each day (~30–40 min).
