...
A) Syntax error   B) Run-time error / exception   C) Logic error   D) Compile-time error

**Answer: C.** It compiles and runs without crashing but produces wrong output — that is a **logic error**. Syntax/compile errors stop compilation; run-time errors throw exceptions.
...
A) `int count = 3.5;`   B) `boolean done = "true";`   C) `double rate = 5;`   D) `int 2x = 4;`

**Answer: C.** `double rate = 5;` widens the `int` literal `5` to `5.0` — legal. (A) loses precision assigning a `double` to `int`; (B) assigns a String to a boolean; (D) has an illegal identifier (starts with a digit).
...
**Answer: A.** `\\` is one backslash; `\"` is one double quote. So the output is `a\b"c`.
...
```
A) (leave blank — `int` defaults to 0)   B) `total = 0;`   C) `int total = 0;`   D) `total == 0;`

**Answer: B.** A **local** variable must be explicitly initialized before use (no default); `total = 0;` assigns it. (A) won't compile — locals have no default. (C) redeclares (duplicate). (D) is a comparison, not an assignment.
...
```
A) An empty line   B) `null`   C) A `NullPointerException` is thrown.   D) `0`

**Answer: B.** Printing a `null` reference prints the text `null`; only **calling a method** on `null` would throw `NullPointerException`.
...
```
A) `4`   B) `3`   C) `3.999`   D) `4.0`

**Answer: B.** `(int)` truncates `2.999 → 2`, then `2 + 1 = 3`. (The cast binds tighter than `+`.)
...
**Answer: A.** `x--` yields the **original** 10 first, then decrements `x` to 9. So `"10" + " " + 9` → `10 9`.
...
A) behaviors; attributes   B) attributes (state); behaviors (methods)   C) packages; libraries   D) constructors; parameters

**Answer: B.** **Attributes** are the object's state (data); **behaviors** are the methods that act on it. An API documents the behaviors a class offers without exposing implementation.
...
A) The method guarantees the array will be non-empty when it returns.   B) The caller must ensure the array is non-empty before calling; the method may misbehave otherwise.   C) The method will throw an exception if the array is empty.   D) The array will be sorted after the call.

**Answer: B.** A **precondition** is what must be true **on entry** — the caller's responsibility. A postcondition is what the method guarantees on return; throwing is not promised unless stated.
...
A) Both call `show(double)`   B) `show(7)` → `show(int)`; `show(7.0)` → `show(double)`   C) Both call `show(int)`   D) It is ambiguous and won't compile.

**Answer: B.** **Overloading** picks the method whose parameter type matches the argument: an `int` literal matches `show(int)`, a `double` literal matches `show(double)`.
...
**Answer: B.** Inner `square(3) = 9`; outer `square(9) = 81`. Control flows into the inner call first, returns 9, then into the outer call.
`[topic 1.9][practice P3]`
...
```
A) `MathUtil.twice(5)`   B) `new MathUtil().twice(5)` is required   C) `twice(5)`   D) `MathUtil.twice()`

**Answer: A.** A `static` method belongs to the **class**, so it is called `ClassName.method(args)` — `MathUtil.twice(5)`. No object is needed (so A, not B). D omits the argument.
...
```
A) `1` to `4` inclusive   B) `0` to `4` inclusive   C) `1` to `5` inclusive   D) `0` to `3` inclusive

**Answer: A.** `Math.random()` ∈ [0.0, 1.0); `* 4` ∈ [0.0, 4.0); `(int)` → 0,1,2,3; `+ 1` → 1,2,3,4. Inclusive range **1 to 4**.
...
A) A class is the same thing as an object.   B) Every class implicitly extends `Object`, so it inherits methods like `toString` and `equals`.   C) A reference variable stores the object's data directly.   D) `null` is a valid object.

**Answer: B.** All classes extend `Object` (inheriting `toString`/`equals`). A **class** is the blueprint; an **object** is an instance; a reference variable stores the **address** of an object, and `null` means "no object".
...
**Answer: A.** `new Point(3, 8)` runs the constructor with `a=3, b=8`, setting `x=3, y=8`. `getX()` returns `x` = `3`.
...
A) `Dog d = Dog("Rex");`   B) `Dog d = new Dog("Rex");`   C) `Dog d = new Dog();`   D) `Dog d = new Dog(Rex);`

**Answer: B.** `new` plus the constructor matching a `String` argument: `new Dog("Rex")`. (A) omits `new`; (C) calls the no-arg constructor; (D) treats `Rex` as an undeclared variable.
