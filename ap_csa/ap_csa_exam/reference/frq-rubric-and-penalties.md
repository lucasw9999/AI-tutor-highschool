...
1. **Blind-grade an officially-scored sample response** before your FRQ self-scores feed the readiness bar. Use either:
   ...
   - a **2025 officially scored sample** — `ap25-apc-computer-science-a-q1.pdf` through `-q4.pdf` (real student responses with the official awards and reader commentary), graded against `ap25-sg-computer-science-a.pdf`; `ap25-cr-report-computer-science-a.pdf` and `ap25-computer-science-a-scoring-statistics.pdf` add context. **2024 and 2023 packages are posted too** — `ap24-apc-computer-science-a-q1.pdf` … `-q4.pdf` and `ap23-apc-computer-science-a-q1.pdf` … `-q4.pdf`. With the Q1/Q3/Q4 filter that is **9 rubric-plus-award pairs across three years**, which is enough to re-test this gate every 8 weeks on material you have never graded before — the only way the ±1-point check stays honest instead of becoming a memory test.
     - ⚠️ **Filename trap when you fetch the question papers.** The 2024 and 2023 **FRQ files** use a shortened stem — **`ap24-frq-comp-sci-a.pdf`** and **`ap23-frq-comp-sci-a.pdf`**, *not* `…-computer-science-a.pdf`. Every other artifact in those years uses the long stem. A 404 on a constructed URL here means the stem is wrong, **not** that the document is missing. Full table in `question-sources-and-access.md`.
...
**Re-verified 12 August 2026 — the whole 2026 scoring package is still absent, for both subjects.** `ap26-sg-*`, `ap26-cr-report-*`, `ap26-apc-*-q1` and `ap26-*-scoring-statistics` all return 404, and AP Central's past-exam-questions pages list **2026 as free-response questions only** — for AP CSA *and* AP Precalculus. That pattern is expected (questions post in the summer, the scoring package follows in the fall) and it means **nothing here is overdue or lost.** Keep watching; do not re-litigate.

**And the artifact class that is NOT missing but was never named here:** the per-subject **score-distribution PDF**, e.g. `ap25-computer-science-a-score-distributions.pdf` — "2025 Student Score Distributions – AP® Computer Science A" (live, verified 12 August 2026). Posted per subject per year, it carries the official 1–5 spread the readiness thresholds are ultimately calibrated against. The 2026 edition (`ap26-computer-science-a-score-distributions.pdf`, 404 today) is the first curve measured on the redesigned exam — it is the recalibration trigger in `../exam-skill-tracker.md` §(f) criterion F.

Until the 2026 rubric lands, this file reflects the rubric structure from the CED sample FRQ rubrics (MessageBuilder Q1, CupcakeMachine Q2, ItemInventory Q3, Schedule Q4), the **2025 scoring guidelines and scored samples** (Q1/Q3/Q4 — with AP Central's caveat that the 2023–25 materials "do not completely align with the current AP CSA Exam"), and the 2026 released FRQs (Account Q1, Bottle Q2, Attendance Q3, GameBoard Q4), which together are the best available rubric sources for the redesigned exam.
