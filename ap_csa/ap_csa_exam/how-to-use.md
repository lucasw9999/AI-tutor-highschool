...
- Two hard deadlines this phase:
  ...
  - **Register to sit the exam by ~mid-November 2026** — register through your school's AP Coordinator. **Confirm early that the school actually administers the AP CSA exam** (a small/private school may not offer every AP); if it doesn't, ask coordinators at nearby authorized schools about an **exam-only section** (the AP Course Ledger can suggest candidates — with the caveats in the deadline table at the end of this file). No deadline, no May 2027 seat. (See `reference/question-sources-and-access.md`; for every fee and date, [`../../ap_precalc/README.md`](../../ap_precalc/README.md) → **MONEY AND DEADLINES**.)
...
| **Register to sit the exam** | **Nov 13, 2026, 11:59 PM ET** is the final deadline. **Oct 2, 2026 is the *preferred* deadline — missing it costs nothing**; the $40-per-exam late-order fee only starts **Nov 14**. Every fee and date for both exams: [`../../ap_precalc/README.md`](../../ap_precalc/README.md) → **MONEY AND DEADLINES**, deliberately the only place in the repo that prints figures. | **You cannot order directly** — only an AP coordinator at a hosting school can. Confirm the school administers AP CSA; if not, ask coordinators about an **exam-only section**. Schools set earlier local deadlines. The **AP Course Ledger** (`https://apcourseaudit.inflexion.org/ledger/`) can list schools that have offered AP CSA, but it is **refreshed each November**, so an Aug/Sept 2026 lookup shows *2025-26* offerings — and a later check found it behind a College Board **professional** login a parent does not have. **The only instrument that answers "does this school administer AP CSA in May 2027" before Nov 13 is a direct question to the AP Coordinator, in the first week of school.** ⚠️ **PLAUSIBLE — UNVERIFIED:** the late path for students whose school doesn't offer the exam — added after Nov 13 *without* the late fee if the coordinator contacts AP Services and the student is in the order by **Mar 12, 2027** — **could not be confirmed on 12 August 2026.** College Board's "Special Ordering Circumstances" page is a link index deferring to the AP Coordinator's Manual Part 1, and the exceptions the fees page *does* name are different ones (courses beginning after Nov 13; transfer students). **Plan as though Nov 13 is the wall**, and have the coordinator check the Manual before relying on it. *(Verified 12 August 2026 except where marked unverified.)* |
